...
public interface IntSpinner extends Spinner<Integer> {
    ...
    /**
     * 細かい値の操作をするときのモディファイア(倍率)を{@code float}で設定します。<br>
     * 例えば、マウスの移動した距離に掛け合わされ、マウス移動による値の変位が変化し、値を細かく調整できるようになります。<br>
    ...
    /**
     * 通常の値の操作をするときのモディファイア(倍率)を{@code float}で設定します。<br>
     * 例えば、マウスの移動した距離に掛け合わされ、マウス移動による値の変位が変化します。<br>
    ...
    /**
     * 大きい値の操作をするときのモディファイア(倍率)を{@code float}で設定します。<br>
     * 例えば、マウスの移動した距離に掛け合わされ、マウス移動による値の変位が変化し、値を大きく調整できるようになります。<br>
